...
customList.value = [
  {
    peopleCount: props.detailData?.techniqueListElectricity.worker_pro,
    title: "ИТР(Инженерно-технические работники)",
  },
  {
    peopleCount: props.detailData.techniqueListElectricity.worker,
    title: "Рабочие",
  },
];
</script>
...
<template>
  <Provider :is-server="true">
    <template
      v-for="(table, index) in detailData.workingTypeElectricity"
      :key="index">
    ...
    <el-table
      :data="detailData.planSmrListElectricity"
      highlight-current-row
      stripe
      border
      class="mb-40">
  ...
  <el-row
    ...
    <el-col
      ...
      <el-table
        :data="detailData.techniqueListElectricity.data"
        highlight-current-row
        stripe
        border>
        <el-table-column
