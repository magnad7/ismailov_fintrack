...
const ruleForm = reactive<StatementType>({
  active: true,
  template_document: {},
  title_en: "",
  title_ru: "",
...
watch(id, async (): Promise<void> => {
  if (id.value) {
    const { data } = await GET<StatementType>(
      `admin/statement-type/${id.value}`
    );
  ...
  for (const key in ruleForm) {
    if (key === "template_document") {
      ...
      if (ruleForm.template_document) {
        if (
          ruleForm.template_document instanceof File ||
          typeof ruleForm.template_document === "string"
        ) {
      ...
      const val = ruleForm[key as keyof StatementType];
